-- Per-employee attendance exemption — for staff (e.g. someone "in charge")
-- whose punches are still recorded when they happen, but who should never be
-- flagged Late or Absent for not punching or punching outside their shift
-- window. Distinct from company-wide Week-off (20260812090000): this is a
-- standing per-person exemption set by the admin, not a scheduled day off.

alter table employees add column if not exists attendance_exempt boolean not null default false;

-- Mirrors the is_week_off branch in calc_payroll_fields (20260806110000): an
-- exempt employee's check-in/out are simply never compared against their
-- shift window for lateness/early-departure, while total_hours/overtime are
