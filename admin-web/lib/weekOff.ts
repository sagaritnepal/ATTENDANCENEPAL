...
export async function fetchMyCompanyWeekOffConfig(): Promise<CompanyWeekOffConfig> {
  ...
  if (!auth.user) return { companyId: null, companyName: null, weeklyOffDay: null, rosterMode: 'monthly' };
  const { data: profile } = await supabase.from('profiles').select('company_id').eq('id', auth.user.id).single();
  const companyId = profile?.company_id ?? null;
  if (!companyId) return { companyId: null, companyName: null, weeklyOffDay: null, rosterMode: 'monthly' };
  const { data: company } = await supabase.from('companies').select('name, weekly_off_day, roster_mode').eq('id', companyId).single();
  return {
    companyId,
